---
title: "ISUSIE: Probabilistic Fine-mapping of Causal Genes"
shorttitle: "ISUSIE"
author:
...
vignette: >
  %\VignetteIndexEntry{ISUSIE: Probabilistic Fine-mapping of Causal Genes}
...
For a comprehensive description of the probabilistic framework behind ISUSIE 
please refer to: 
...
studies and colocalization-sum of single effects (ISuSiE), a method for causal 
gene fine-mapping and effect estimation that efficiently estimates informative 
priors. 

# Included Data Sets

To demonstrate the functionality of the `ISUSIE` package, we include several 
...
from fastENLOC colocalization analysis, but the user is free to specify 
...
prior. We compute the gene priors using the function `isusie_priors`:
...
ISUSIE::isusie_priors(GRCP_vec = grcp, p12 = p1_p12[2],TWAS_pi = twas_pi1,
                      n_cis = n_cis)
...
we utilize the `isusie_null_wt` function:

```{r}
data(sbams)

ISUSIE::isusie_null_wt(p1 = p1_p12[1], p12 = p1_p12[2], 
...
for causal gene fine-mapping. To run ISuSiE, we use the `isusie` function.
...
The `isusie` function is designed to perform causal gene fine-mapping for one
